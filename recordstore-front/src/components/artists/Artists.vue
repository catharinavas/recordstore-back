<template>
  <div class="max-w-md m-auto py-10">
    ...
    <form @submit.prevent="addArtist">
      ...
      <input type="submit" value="Add Artist" class="font-sans font-bold px-4 rounded cursor-pointer no-underline bg-green-500 hover:bg-green-600 block w-full py-4 text-white items-center justify-center">
    </form>
        ...
        <div v-if="artist == editedArtist">
          <form @submit.prevent="updateArtist(artist)">
            <div class="mb-6 p-4 bg-white rounded border border-gray 200 mt-4">
              ...
              <input type="submit" value="Update" class="my-2 bg-transparent text-sm hover:bg-blue-500 hover:text-white text-blue-500 border border-blue-500 no-underline font-bold py-2 px-4 rounded cursor-pointer ">
            </div>
          </form>
        </div>
      </li>
